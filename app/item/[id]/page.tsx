...
export default async function ItemsDetails({
  ...
  return (
    <div className="min-h-screen flex mx-auto max-w-5xl items-center pt-20 pb-8 gap-12">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2 text-sm font-medium text-black/80 dark:text-white/80 leading-4 tracking-tight">
          {newUsed}&bull;<p>{item.type}</p>&bull;<p>{item.uni}</p>&bull;
          <p>
            By {data.user?.id === item?.uploadById ? "you" : item.uploadedByName}
          </p>
        </div>
